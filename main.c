...
int main(int argc __attribute__((unused)), char **argv, char **environment)
{
	...
	info.init_args = argv;
	info.env_vars = create_shell_environment(enviroment);
	...
	while (getline(&(info.cmd_buffer), &buffer_len, stdin) != -1)
	{
		...
		{
			info.args = tonize_command(info.cmd_list[i], "\n \t\r");
			if (info.args && info.args[0])
			{
				if (indentify_shell_builtin(&info) == NULL)
	...
	if (is_pipe == 0)
		print_custom_string("\n");
	destroy_shell_enviroment(info.env_vars);
	free(info.cmd_buffer);
	exit(info.exit_code);
}
